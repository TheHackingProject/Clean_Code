...
Genre c'est dur, personne ne comprends rien, la demande à été formulée à l'arrache en fin d'une réunion ou personne ne voulait être.
...
Avec une telle description tu comprends la raison pour laquelle RSpec à choisi de s'orienter sur du BDD.
...
Personnellement j'ai même vu RSpec utilisé pour généré les rapports de preuve de sécurité par design pour une appli médicale.
...
Dans ces quelques lignes de code qui ferait pleurer du sang à à n'importe quel dèv sénior (je vais l'appeler Guillaume par la suite, mais donne lui un petit nom à toi 😉️), on pourrait imaginer tester ce que fait la fonction quand elle reçoit un string. A priori elle renvoie `false`.
...
Imagine également que la personne avant toi, réelle Stakhanoviste distinguée du testing, ai prévue absoluement TOUS les cas de figures...
...
La version réelle que tu risque de recontrer : c'est que ce n'est pas testé. C'est bête mais c'est ça de moins à faire, à maintenir, et comme le code est utilisé en interne, on suppose (hum) que tu as documenté cette méthode quelque part. Donc le développeur suivant saura comment s'en servir.
...
Bien sûr une méthode peut faire tout ça en même temps. Mais c'est peu recommandé, si tu en trouves une parles-en à ton ou ta lead dèv.
...
Sauf que cette valeure de retour est à décider entre plusieurs cas possibles.
...
# C'est le code qui permet à un user de quitter un project.
  ...
  # Bien sûr il faut que tu imagins qu'il existe un controlleur quelque part, ou un model, qui vient modifier la valeur de cet attribut.
end
```
...
On fait du test unitaire et on teste, nous, la méthode `remove_member`. Cela veut dire qu'on ne se préocuppe pas de savoir si les attributs et méthodes `locked`, et `creator` fonctionnent correctement. On va juste partir des cas différents qu'elles peuvent renvoyer.
...
Car on pourrait très bien tester tous les cas possible ci-dessus de manière brutale. Mais ça ferait 2 puissance 3 cas différents soit 16 tests avec un cas de figure différent à écrire pour chaque test (revérifie, mais normalement je ne me suis pas trompé).
...
C'est la que la théorie de Boole, et les diverses théories ensemblistes interviennent. Ne paniques pas, je suis juste en train de faire mon malin. Avec un schéma tout le monde sait faire ça.
...
Pour bien comprendre je vais écrire mes tests en Markdown. Pour l'instant ils ressemble à ça :
```markdown
# Class Project
...
Si tu ne te poses pas la question, ce n'est pas grave nous l'aborderons plus en détails dans la semaine, tu peux donc sauter cette section.
