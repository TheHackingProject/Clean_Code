...
En gros cette ressource va t'apprendre à formatter ton cerveau pour réfléchir `test`.
...
Le BDD c'est pour `Behavior` driven development. Je décris un comportement. Je décris comment mon bout de code est sensé se comporter face à telle situation. Puis alors j'écris le code en question.

### 2.1 Pourquoi BDD ?
Si tu as déjà eu à faire à un client, un lead, ou à un cahier des charges, une issue github, un ticket Jira ou autre, tu as bien vu que souvent.... c'est la merde. Le ticket n'est pas assez détaillé.
Il manque généralement des infos du genre : "oui mais dans ce cas là on fait quoi ?".
...
En gros l'idée c'est d'écrire le cahier des charges sous forme de tests. Et, avec des tests orientés sur le comportement de l'appli, c'est uniquement si le comportement souhaité change que l'on va changer les tests ! Pas lorsqu'on a oublié un cas tordu de test d'un type de variable inatendu etc...
...
Avec une telle description tu comprends pour RSpec fait du BDD.

### 2.2 La manière RSpec
Contrairement à ce que tu pourrais croire à ce stade, RSpec à été créé pour rendre la vie des dèv plus simple. Pas pour casser les pieds de moussaillons en PLS sur un projet chaton.
...
Car non écrire des tests n'est pas le but d'RSpec en fait. Le but d'RSpec c'est d'obtenir le bénéfice de cette écriture. Et non Jean-michel Obvious... ce n'est pas juste pour "éviter les bugs".
Non écrire des tests c'est pour :
- Créer de la confiance en votre code et l'avancée de votre projet.
- Eliminer les peurs que le moindre changement va tout péter.
- Mieux, ça permet de faire un gros refacto, car si les comportement (behavior) sont les mêmes, alors les tests aussi.
...
Le TDD est maintenant un peu mainstream et ça ne te fera pas briller en soirée dansante (laisse tomber j'ai essayé).
Le truc c'est que les gens ont tendance à trop tester.
...
Dans ces quelques lignes de code qui ferait pleurer du sang à Guillaume Faure-Dumont (bisous à toi si tu me lis), on pourrait imaginer tester ce que fait la fonction quand elle reçoit un string. A priori elle renvoie `false`.
Mais doit-on le tester ? Dans ce cas est-ce qu'on test aussi ce qu'il se passe quand l'argument est un `Object`, `Float` ou `Array` ?
On pourrait.
Imaginons qu'un utilisateur utilise `is_3?('3')`... on aurait légitimement envie de dire que la méthode devrait renvoyer `true`...

Et là, sans faire gaffe vous venez de tomber dans le rabbit hole du "Je teste tout et beaucoup trop, ce qui m'amène à refactor ma méthode 15 fois sans jamais pouvoir la ship".

Imagine également la personne avant toi, réelle Stakhanoviste distinguée du testing ai prévue absoluement TOUT les cas de figures...
...
Mais imagine maintenant que cette méthode soit uniquement utilisée en interne du code par d'autres développeurs. C'est ptet pas la peine d'aller écrire cent tests différents pour tous les cas.
On prend une classe au pif... celle qui te fait marrer, mettons `String`. Et puis tu tests que la méthode renvoie bien `false` si on lui donne la chaine de caractère `'3'`.
Basta.
...
Ce serait plus dur... mais en cas de changement, il n'y aura qu'à modifier cette collection.

La version réelle que tu risque de recontrer : c'est que ce n'est pas testé. C'est bête mais c'est ça de moins à faire, à maintenir, et comme le code est utilisé en interne, on suppose (hum) que tu as documenter cette méthode quelque part. Donc le développeur suivant saura comment s'en servir.
...
Mais surtout sa particularité c'est de n'avoir qu'une valeure de retour.
...
Celles qui provoque aussi un effet de bord comme celle là :
```ruby
...
```
Cette méthode return directement son argument d'entrée elle ne fait donc rien.
A ceci près que si le paramètre d'entrée est `nil` alors la méthode provoque une erreur.
C'est un effet de bord. Celui là est clairement visible. D'autre seront plus discrets. Il faudra être attentif à cela.

Bien sûr une méthode peut faire tout ça en même temps. Mais c'est peut recommandé.
...
C'est un cas particulier et vachement tordu du cas où la méthode n'a qu'une valeur de retour. Sauf que cette valeure de retour est à décidé entre plusieurs cas possibles.

Observons cette méthode directement issue du code en production de THP à l'heure où j'écris ces lignes :
```ruby
class Project
# le code classique d'un model rails, relation has_many user as member, et méthodes locked, creator.
# C'est le code qui permet à un user de quitter un project.
  def remove_member(user)
    ...
    (errors[:base] << "Le createur ne peux pas quitter son projet !") and return false if user == creator
...
```
Pendant que Guillaume commence à arroser d'essence l'intérieur du monastère, regardons de plus près cette méthode.
Cette méthode modifie deux variables existantes : `errors` et `members` et renvoient true ou false suivant les cas.
...
Je ne peux pas tout te montrer du code, mais en gros, les méthodes `locked` renvoient true ou false, tandis que la méthode `creator` comme tu l'imagines renvoie un user.

On fait du test unitaire et on teste, nous, la méthode `remove_member`. Cela veut dire qu'on ne se préocuppe pas de savoir si les méthodes `locked`, et `creator` fonctionnent correctement. On va juste partir des cas différents qu'elles peuvent renvoyer.
...
- Le cas ou user n'est pas inclu dans member.

Ca en fait pas mal des cas...

La question donc c'est : doit-on le tester ?
Car on pourrait très bien tester tous les cas possible ci-dessus de manière brutale. Mais ça ferait 2 puissance 3 cas différents soit 16 tests avec un cas de figure différent à écrire pour chaque test.

Or si on regarde bien, le cas `user == creator` n'a pas lieu d'exister si `locked == true`, normalement la méthode n'y passe jamais.
Je te laisse réfléchir là dessus : c'est important que tu fasses l'effort de comprendre pourquoi. Demande aux autres de ton groupe si besoin.
...
C'est la que la théorie de Boole, et les diverses théories ensemblistes interviennent. Tu paniques pas je suis juste en train de faire mon malin. Avec un schéma tout le monde sait faire ça.
Regarde, ça c'est l'ensemble des projects possibles:
<img src="projects.png">
et un project peut être, soit locked (en bleu), soit `locked == false` en vert.

Donc si tu es dans la zone bleu : peut importe le reste.

Si tu es dans la zone verte maintenant on a le cas où le user est creator, et celui où il n'est pas creator.
![projects_not_locked](projects_not_locked.png) Dans ce cas, il n'y a pas besoin de tester ce qu'il se passe quand on est locked ou pas, c'est déjà traité depuis tout à l'heure !

Pour l'instant tes tests ça va ressembler à ça :
```markdown
# Class Project
...
### Dans le cas où le project n'est pas locked
  ...
  on s'attend à ce que la méthode renvoie `false` et que `errors` contienne la chaine `"Le createur ne peux pas quitter son projet !"`.

  #### Dans le où le user n'est pas le creator
  on s'attends à ce que la méthode renvoie `true`
```
...
Voilà... tu as donc fait 5 tests avec autant de cas de figures différents. C'est long, c'est relou, mais nous sommes loin des 16 tests qu'on aurait pu imaginer au début.
Au passage : félicitation, tu viens de manipuler les théories ensemblistes (t'as vu c'est fastoche hein ?).

Pour info les dessins ont été fait avec [Excalidraw.io](https://excalidraw.com/) que je te recommande si tu ne connaissais pas.



### 3.5 Les autres types de tests
Juste pour que tu ne paniques pas si tu te pose la question je vais aborder les autres types de tests.
...
On test chaque méthode séparément, en admettant à chaque fois que les autres méthodes fonctionnent parfaitement.
Cela permet de vérifier exactement à chaque micro étape du projet, que tout est bien conforme à ce qui est attendu.

Mais même si tout fonctionne séparément, cela ne garanti pas que tout fonctionne ensemble.
...
Cette fois ci tu test une autre partie du code que généralement tu ne possèdes pas. Comme par exemple une API. Tu vas écrire des tests qui permettent de t'assurer par exemple que l'API tierce fonctionne bien comme la documentation l'indique.
L'idée derrière ça c'est surtout :
- De documenter comment fonctionne l'API au sein de ton propre code
- De vérifier que l'API n'a pas changé suite à une mise à jour de la mort qui t'aurais échappé.
...
A la fin de ce cursus d'une semaine je t'en recommande chaudement la lecture, c'est le doudou que sert fort Guillaume en regardant les braises de son monastère.


## 4. Points importants à retenir
On a vu ce que c'était que les tests unitaires : des tests qui cible de manière très fine une et une seule méthode à la fois.
...
On a vu qu'il faut réfléchir à ce que fait réellement une méthode : quelles valeures elle retourne, a-t-elle un effet de bord, modifie-t-elle la valeure d'une variable, mais surtout dans quels cas.
On a vu également qu'il faut surtout réfléchir à ce qu'il n'est PAS nécessaire de tester. Soit parce que le scope de la méthode est a peu près certains, où que les cas seraient trop nombreux au quel cas il faut essayer de rationnaliser les efforts par rapport au risque pris et aux personnes concernés par ce bout de code (utilisateur final, ou développeur en interne).
