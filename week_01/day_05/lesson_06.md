...
Cela s'emploie bien quand on veut simuler une requête (HTTP par exemple) ou une méthode qui doit renvoyer une valeure sans avoir d'effets de bords.
...
Et comme tu l'auras préssenti, on peut aussi le faire en deux temps, avec la commande `allow` :
...
### 3.3 Mocks
Ici, ce n'est pas la valeure de retour que l'on souhaite simuler mais au contraire l'effet de bords.
Le mock va juste vérifier que la méthode `jump` a bien été appellée.
A la fin du test, RSpec va vérifier que les mocks ont bien été appellés, et renvoyer une erreur si ce n'est pas le cas, faisant ainsi planter votre test.
...
```
Ici on test simplement que si on appuie sur le bouton A, cela appelle la méthode `:jump` de l'objet `mario`. Mais on se fout complètement de la valeur de retour de la méthode `jump`.
...
Bien sûr tu n'auras pas à lancer la vérification `RSpec::Mocks.verify` dans tes tests. Ce sera lancé automatiquement à la fin de chaque example par RSpec.

### 3.4 Null Object
Les doubles que l'on a vu jusqu'ici sont dit "strictes". C'est à dire qu'il est nécessaire de déclarer à l'avance les méthodes autorisées ou non. Mais quelques fois on a besoin que le double puisse accepter n'importe quelle méthode sans broncher.
C'est le cas des null objects qui renverront systématiquement eux même quand on leur passe n'importe quelle méthode.
...
### 3.5 Spies
C'est l'objet le plus subtile de la liste.
...
Mais dans ce cas, RSpec a penser à toi, et t'offre la méthode `spy` directement :
...
Cette section est un peu plus avancée et tu n'en a pas besoin pour faire le projet du jour.
...
### 4.2 Partial doubles
A l'inverse, souvent, les objets ont déjà été codés et il ne leur manque plus qu'une fonctionnalité ou deux. Ou encore, les objets existent mais on ne peut contrôller leur valeur de retour au sein du test : `Time.now`, ou `Random.new` par exemple.
...
# toutes les autres méthodes de l'objet random fonctionneront comme indiquées dans la documentation de Ruby.
```

### 4.3 Verifying doubles
L'avantage jusqu'alors c'est que nos doubles pouvaient prendre la forme que l'on voulait. Le problème avec ça, c'est que souvent, comme ton code va évoluer, tes tests pourraient ne plus reflêter la réalité.
Les Verifying doubles peuvent se prémunir de cela en "vérifiant" littéralement si l'objet en question en est bien capable.

Pour donner un exemple, tout à l'heure on a utiliser `peach` et on a chercher à vérifier qu'elle recevait bien la méthode `:jump`. Mais imaginons que maintenant qu'on a coder la class Character, ce n'est plus `:jump` mais `:bounce` qu'on utilise.

Eh bien tes tests continueront de passer. Puisque tu simule le comportement de peach.
...
Note que, dans un tel cas, avec un double classique, ton test unitaire continuait à passer, mais normalement ton test d'acceptance aurait planter. Donc ce n'est pas si grave que cela.
...
Je vais pas y aller par 4 chemins, un petit snippet te montrera comment ça marche.
```ruby
class Maths
 CONSTANT_PI = 3.14
end
...
Dans cette ressource, on a appris à utliser les différents types d'objets offerts par RSpec et on a appris non seulement à les différencier, mais également à les utiliser.
Il faut retenir que
- double est la méthode qui permet de définir ces objets.
- Stub : renvoie une valeur prédéfinie à une méthode prédéfinie.
- mock : vérifie qu'une méthode à bien été appellée s'assurant ainsi de l'effet de bord plutôt que de la valeur de retour.
- null_object : renvoie lui même quoiqu'on lui fasse.
- spy : vérifie les appels passer ou non à un objet réel après coup.

## 5. Pour aller plus loin
Pas de mystères :
- la documentation d'RSpec,
