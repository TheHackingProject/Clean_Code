...
En soit : pouvoir se logger est une fonctionnalité, un comportement.

Mais en somme, cela ne va pas plus loin qu'un test controller :
- On vérifie qu'avec les bon paramètres le controller renvoie une 200.
...
Dans ce module on va déjà créé la méthode qui permet de définir le `current_user` en rajoutant ces quelques lignes :
...
Voilà, donc si jamais tu include ce module dans un fichier spec, tu auras accès à la méthode `current_user` qui renverra le User correpondant à l'`id` si current_user n'est pas déjà défini.
...
Voilà, comme ça ton module sera inclu systématiquement dès que tu mettras la metadata `type: :request` à ton fichier de specs.
...
- Soit un méchanisme d'authentification en DIY dont tu as un exemple ici.
